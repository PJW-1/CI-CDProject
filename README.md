...
정직하게 밝힙니다. 아래는 아직 하지 않았습니다.

- `ruff format` 미적용 (16개 파일 재포맷 대상, CI는 `continue-on-error`)
- 의존성 락파일(pip-tools) 미도입 — 핀 정합성만 확인
- `tracemalloc` 장시간 메모리 프로파일링 미실시
- 손 포함 프레임 벤치마크 미자동화 (카메라 필요)
- 브랜치 보호 규칙 미적용 — 현재 CI가 빨간색이어도 머지 가능
- 실제 클라우드 배포 — CD는 빌드·기동 검증까지
